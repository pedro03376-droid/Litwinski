...
1. Abra o **link** no navegador (funciona no computador e no celular).
2. Clique em **Entrar com o Google** e escolha sua conta.
   - ⚠️ Use o **login com Google** — é ele que permite salvar os dados na nuvem.
3. Pronto: você já está no sistema.

> Dica: no celular, use a opção do navegador **"Adicionar à tela inicial"** para abrir como um aplicativo.

---

## 2. Cada clube tem seus próprios dados (isolados)

Os dados de **clubes diferentes NUNCA se misturam**. Isso é garantido por um
**código de clube** (em *Config. do Clube → 🔑 Código do clube*).
...
- Peça o **código do clube** a quem já usa o sistema.
- Vá em **Config. do Clube → 🔑 Código do clube → "Entrar em outro código…"** e cole o código.
- O app recarrega e passa a **sincronizar com o mesmo clube**.

---

## 3. Salvar e sincronizar

- Os dados ficam salvos **no seu aparelho** e sobem para a **nuvem** automaticamente.
- Para forçar: **Config. do Clube → 🔄 Sincronizar agora**.
- **🟢 Online / 🟡 Sincronizando / 🔴 Offline** aparece no topo. Offline você continua trabalhando; sincroniza sozinho ao voltar a internet.
...
- App novo demorando a aparecer? Recarregue com **Ctrl + Shift + R** (ou Cmd + Shift + R no Mac).
- Erro de sincronização? Confirme que entrou com o **Google** e que está **online**.
